...
`var`命令存在变量提升问题，变量可以在声明之前使用，值为`undefined`。
...
而`let`命令不存在变量提升问题，变量在声明之前使用会报错。
...
在代码块内，使用`let`命令声明变量之前，该变量都是不可用的。这在语法上，称为“暂时性死区”（temporal dead zone，简称 TDZ）。其本质是，只要一进入当前作用域，所要使用的变量就已经存在了，但是不可获取，只有等到声明变量的那一行代码出现，才可以获取和使用该变量。
...
但是要注意，我们应该避免在块级作用域内声明函数。如果确实需要，也应该写成函数表达式，而不是函数声明语句。
